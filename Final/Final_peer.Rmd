...
Dropping the basement quaility variable improves the adjusted r squared value from x to y. Running the selection process again on the candidate model (droped bsmt quality).
...
#plot(coef(model_bas_no_baseq), ask = F)
```
...
sqrt(mean((ames_test$price - exp(pred_test$fit))^2, na.rm=TRUE))
```
...
Yes, as discovered in the above exploritory data analasys the area variable was log-transformed as doing so proved to provide a better fit for the model. This may be due to the ability of the log to manage the wide variance in area values.

The price was also logged. This also leading to a better fit, probably for similiar reasons as stated for the area.
...
The first models variables where selected by their ability to improve the adjusted R squared value of the model under review. This values is indicitive of how well the model fits the data which should lead to better predictions and lower errors.

The final model was defined using the Bayesian Model Averaging uwing the BAS import. This method reduces or removes model coefficients that lead to a lowwer posterior probability of inclusion. Allowing for more information to be preserved. This approach also limits model overfitting by reducing the impact of low posterior probability variables.
...
Looking at the RMSE of the training data:
```{r}
...
Looking at the RMSE of the testing data:
```{r}
...
The model was tested by its ability to reduce the RMSE (Root Mean Square Error). The RMSE for the test data x was less than the RMSE for the training data y so overfitting did not happend so the model has the ability ot generalise.
...
#ggplot(data = plot_data_final, aes(x = fitted, y = resid)) + geom_point() + xlab("Residuals")

```
```{r}
#ggplot(data = plot_data_final, aes(x = resid)) + geom_histogram(bins=50) + xlab("Residuals")
```
```{r}
#ggplot(data = plot_data_final, aes(sample = resid)) + stat_qq()
```
...
Final models training RMSE is:
```{r}
final_rmse_train
```

Final models testing RMSE is:
```{r}
final_rmse_test
```
...
The model does a fair job at predicting house prices. It is about $26000 off the price either way. The model does well in the testing and validation sets, implying that there was little to no over fitting. The model has a few issues with outliers such as trying to forecast large houses.
